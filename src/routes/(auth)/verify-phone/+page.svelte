<script lang="ts">
  ...
  async function verifyEmail() {
    isLoading = true
    const { error, data } = await trpc($page).auth.verifyPhone.query({
      code,
    })
  ...
  async function resendEmail() {
    if (!canResend) return
    const { message } = await trpc($page).auth.resendPhoneVerification.query()
    toast(message)
    startResendTimer()
...
<div class="bg-background flex h-full flex-col items-center justify-center">
  <div class="mx-auto w-full max-w-md space-y-6">
    <div class="text-center">
      <h1 class="text-3xl font-bold">Verifique o seu celular</h1>
      <p class="text-muted-foreground mt-2">
        Digite o código de verificação de 8 dígitos enviado para seu celular. {$user?.phone}
      </p>
    </div>
    <div
